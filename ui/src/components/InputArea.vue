...
<template>
  <div class="input-wrapper">
        ...
        <div class="model-selector" v-if="showModelSelector">
          ...
          <div v-if="isModelDropdownOpen" class="model-dropdown-menu">
            <div v-for="model in modelOptions" :key="model.value" class="model-option"
              :class="{ 'model-option-active': model.value === selectedModel }" @click="selectModel(model.value)">
              {{ model.text }}
            </div>
          </div>
        </div>
    ...
    <input ref="imageInputRef" type="file" accept="image/*" class="hidden-file-input" @change="onImageSelected" />
    <div class="input-surface">
      <textarea class="textarea" v-model="inputContent" placeholder="组织好你的问题，让我为你补全灵感..."
        placeholder-style="color: rgba(47,58,102,0.35)" :disabled="isRecording || isTranscribing"
        maxlength="1000"></textarea>
      <div class="input-actions">
        <div class="icon-btn" @click="toggleRecording" :class="{ active: isRecording, disabled: isTranscribing }"
          title="语音输入">
...
const handleImageUpload = () => {
  if (props.isRecording || props.isTranscribing || props.isAssistantTyping) return;
...
const canSend = computed(() => inputContent.value.trim().length > 0 && !props.isRecording && !props.isTranscribing && !props.isAssistantTyping);
